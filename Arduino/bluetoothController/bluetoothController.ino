...
void loop() {
  ...
  if(serialDevice.available()) { //Checks for Serial connection availability
    ...
    if(blue_data == 'B') { // If Byte data is 'B' write pins LOW to move backward
      digitalWrite(ledPin1,HIGH);
      delay(2500);
    ...
    if(blue_data == 'S') { // If Byte data is 'S' write pins LOW to Stop
      digitalWrite(ledPin1,HIGH);
      delay(2500);    
    }
